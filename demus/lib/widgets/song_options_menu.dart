            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        if (isDownloaded)
                          const Icon(Icons.download_done_rounded, color: Color(0xFF30D158), size: 22),
                      ],
                    ),
                  ),
                  ...
                  _item(ctx, Icons.add, 'Ajouter à la playlist', showChevron: true, onTap: () {
                    Navigator.pop(ctx);
                  ...
                  _item(ctx,
                    ...
                    isLiked ? 'Retirer des Titres likés' : 'Sauvegarder dans Titres likés',
                    iconColor: isLiked ? const Color(0xFFFF2D55) : Colors.white,
                    onTap: () {
                      provider.toggleFavorite(song);
                      Navigator.pop(ctx);
                      _snack(isLiked ? '💔 Retiré des favoris' : '❤️ Ajouté aux favoris');
                    }),
                  ...
                  if (isDownloading)
                    ListTile(
                      leading: SizedBox(width: 28, height: 28,
                        child: CircularProgressIndicator(value: progress, color: const Color(0xFF30D158), strokeWidth: 2.5)),
                      title: Text('Téléchargement… ${(progress * 100).toInt()}%',
                        style: const TextStyle(color: Colors.white70, fontSize: 15)),
                    )
                  else
                    _item(ctx,
                      isDownloaded ? Icons.delete_outline : Icons.download_rounded,
                      isDownloaded ? 'Supprimer le téléchargement' : 'Télécharger',
                      iconColor: isDownloaded ? const Color(0xFFFF453A) : Colors.white,
                      onTap: () {
                        if (isDownloaded) {
                          ...
                          provider.downloadSong(song, onComplete: () =>
                            _snackGreen('✅ "${song.title}" téléchargé !'));
                        }
                      }),

                  // 4. File d'attente
                  _item(ctx, Icons.queue_music_rounded, "Ajouter à la file d'attente", onTap: () {
                  ...
                  // 5. Radio liée
                  _item(ctx, Icons.sensors_rounded, 'Accéder à la radio liée au titre', onTap: () {
                    Navigator.pop(ctx);
                    _snack('📻 Radio : bientôt disponible');
                  }),

                  // 6. Accéder à l'artiste (recherche)
                  _item(ctx, Icons.person_outline_rounded, "Accéder à l'artiste", showChevron: true, onTap: () {
                    Navigator.pop(ctx);
                  ...
                  // 7. Accéder à l'album
                  _item(ctx, Icons.album_outlined, "Accéder à l'album", onTap: () {
                    Navigator.pop(ctx);
                    _snack('💿 Album : ${song.title.split('(').first.trim()}');
                  }),

                  // 8. Afficher les crédits
                  _item(ctx, Icons.receipt_long_outlined, 'Afficher les crédits', onTap: () {
                    Navigator.pop(ctx);
                    showDialog(
                      context: context,
                      builder: (_) => AlertDialog(
                        ...
                        actions: [
                          TextButton(
                            ...
                            child: const Text('Fermer', style: TextStyle(color: Color(0xFFFF2D55)))),
                        ],
                      ),
                    );
                  }),
                  ...
                  // 9. Exclure du profil
                  _item(ctx, Icons.cancel_outlined, 'Exclure de votre profil de goût', onTap: () {
                    Navigator.pop(ctx);
    ...
    builder: (_) => Column(
      ...
      children: [
        ...
        ...provider.playlists.map((pl) => ListTile(
          ...
          title: Text(pl['name'] as String, style: const TextStyle(color: Colors.white)),
...
Widget _item(BuildContext context, IconData icon, String title, {
  VoidCallback? onTap, bool showChevron = false, Color iconColor = Colors.white,
}) {
  return ListTile(
    leading: Icon(icon, color: iconColor, size: 26),
    title: Text(title, style: const TextStyle(color: Colors.white, fontSize: 15)),
    trailing: showChevron ? const Icon(Icons.chevron_right, color: Colors.white38) : null,
    onTap: onTap,
  );
}
